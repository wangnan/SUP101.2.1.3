...
- (SUPObjectList*)salesOrders:(BOOL)ignoreNonPending :(BOOL)ignoreDisableSubmit
{
    if (_salesOrders == nil)
    {
        self.salesOrders = [SUP101Sales_order getSalesOrders_for_Customer:[NSNumber numberWithLongLong:_surrogateKey]];
    }
    return [[self class] filterChildren:_salesOrders:ignoreNonPending:ignoreDisableSubmit];
}
- (SUPObjectList*)salesOrders
{
    return [self salesOrders:NO:NO];
}
...
- (void)setSalesOrders:(SUPObjectList*)newSalesOrders
{
    if (_salesOrders != newSalesOrders)
    {
        [newSalesOrders retain];
        [_salesOrders release];
        _salesOrders = newSalesOrders;
    }
}
...
- (SUPObjectList*)getSalesOrdersFilterBy:(SUPQuery*)query
{
	return [self getSalesOrdersFilterBy:query skip:0 take:INT_MAX]; 
}


	

- (SUPObjectList*)getSalesOrdersFilterBy:(SUPQuery*)query skip:(int32_t)skip take:(int32_t)take
{
...
/*!
  ...
  @throws SUPPersistenceException
 */
- (int32_t)getSalesOrdersSize:(SUPQuery*)query
{
    SUPStringList *list = [SUPStringList getInstance];
    [list add:@"customerFK"];
