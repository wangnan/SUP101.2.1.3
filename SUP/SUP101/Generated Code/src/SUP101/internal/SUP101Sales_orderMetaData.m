...
- (id)init
{
    if (self = [super init]) {
		...
		[o_findByPrimaryKey_1 setIsDelete:NO]; 		
 		SUPOperationMetaData* o_getSalesOrders_for_Customer_1 = [SUPOperationMetaData createOperationMetaData:6:(SUPString)@"getSalesOrders_for_Customer":[SUPDataType forName:@"Sales_order*"]:true];
	  	{
			SUPObjectList *parameters_list = nil;
	 		SUPParameterMetaData* p_getSalesOrders_for_Customer_surrogateKey = [SUPParameterMetaData createParameterMetaData:1:(SUPString)@"surrogateKey":[SUPDataType forName:@"long?"]];
 			parameters_list = [SUPObjectList listWithCapacity:1];
			[parameters_list addThis:p_getSalesOrders_for_Customer_surrogateKey];
			o_getSalesOrders_for_Customer_1.parameters = parameters_list;
	 	}
		[o_getSalesOrders_for_Customer_1 setIsStatic:YES];
		[o_getSalesOrders_for_Customer_1 setIsCreate:NO];
		[o_getSalesOrders_for_Customer_1 setIsUpdate:NO];
		[o_getSalesOrders_for_Customer_1 setIsDelete:NO]; 		
		...
		[o_getSize_1 setIsDelete:NO]; 		
 		...
 		[operations addThis:o_getSalesOrders_for_Customer_1];
		...
		a_customer.inverseAttribute = @"salesOrders";
